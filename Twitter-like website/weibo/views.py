...
def send(request):  # 发布微博，并刷新页面
    if request.method == 'POST':
        ...
        if(len(mycontent.content)>150):
            return HttpResponse('your content words is large than 139')
...
def sendcomment(request):  # 发布评论
    ...
    if(len(newcomment.comment)>100):
        return HttpResponse("your comment is large than 100")
